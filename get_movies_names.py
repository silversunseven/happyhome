...
def chk_for_R(mpaa, title, file_loc, table_data_rating, dict_param):
    ...
    if "Rated R" in mpaa and "language" in mpaa or "Rated PG-13" in mpaa and "language" in mpaa:
        ...
        table_data_rating.append([dict_param['rating'],dict_param['title'],dict_param['poster'],dict_param['year'],dict_param['mpaa'],dict_param['plot'],dict_param['genres'],dict_param['runtime'],dict_param['trailer'],dict_param['IMDB_query']])
...
def write_rating_html(table_data_rating,dict_param):
    ...
    htmlcode = HTML.table(sorted(table_data_rating, reverse=True), header_row=['Rating','Title','Poster','Year','MPAA','Plot','Genres','Runtime','Trailer','IMDB'])
    table_data_rating.append([dict_param['rating'],dict_param['title'],dict_param['poster'],dict_param['year'],dict_param['mpaa'],dict_param['plot'],dict_param['genres'],dict_param['runtime'],dict_param['trailer'],dict_param['IMDB_query']])
    ...
    for result in resList:
        ...
        if file_year == '' and ti['kind'] == 'movie':
            ...
            if dict_param['mpaa'] == 'None':
                ...
                [dict_param['rating'],dict_param['title'],dict_param['poster'],dict_param['year'],dict_param['mpaa'],dict_param['plot'],dict_param['genres'],dict_param['runtime'],dict_param['trailer'],dict_param['IMDB_query']],)    
                break
            else:
                chk_for_R(dict_param['mpaa'],dict_param['title'], file_loc,table_data_rating,dict_param)
                break
...
def get_files(ftype,table_data_rating,dict_param):
    ...
    for filename in list_of_files:
        ...
        if movie:
            ...
            query_movie_name(clean_txt3mov, clean_txt2year, movie.group(0),table_data_rating,movTrailerURL,movPosterURL,dict_param)
